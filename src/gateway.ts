...
class Channel {
  ...
  private fromAgent(line: Buffer): void {
    ...
    if (isResponse(f)) {
      ...
      // this session's parked frames. Keyed by the gateway id rather than by
      // `origin`, and resolved before the origin lookup because its origin is a
      // sentinel with nothing to route to.
