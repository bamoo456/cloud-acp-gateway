...
import { makeTestServer, Gateway, TASK_TTL_MS, SESSION_IDLE_TTL_MS, handleRequest, type Conn } from "./gateway.ts";
...
test("emitRawSDKMessages _meta is injected into session/new and session/load only for the claude adapter", async () => {
  // The gateway needs a raw system-init message to learn what model a resumed
  // session is actually running (see handleSdkMessage below) — but only
  // claude-agent-acp understands the _meta flag that asks for it, and the
  // gateway detects that from the initialize response's
  // agentCapabilities._meta.claudeCode, not from the channel's name.
  const { port, agent, close } = await makeTestServer();
  const a = sse(port);
  const conn = await a.conn;

  await post(port, conn, { jsonrpc: "2.0", id: 1, method: "initialize", params: { protocolVersion: 1 } });
  const initFwd = agent().sent.map((s) => JSON.parse(s) as Msg).find((o) => o.method === "initialize");
  agent().emit(Buffer.from(JSON.stringify({
    jsonrpc: "2.0", id: initFwd!.id,
    result: { protocolVersion: 1, agentCapabilities: { loadSession: true, _meta: { claudeCode: {} } } },
  })));
  await new Promise((r) => setTimeout(r, 20));

  // session/new: the client's own _meta (top-level field + a nested claudeCode
  // field) must survive the merge untouched.
  await post(port, conn, {
    jsonrpc: "2.0", id: 2, method: "session/new",
    params: { cwd: "/proj", mcpServers: [], _meta: { mine: true, claudeCode: { promptQueueing: true } } },
  });
  const newFwd = agent().sent.map((s) => JSON.parse(s) as Msg).find((o) => o.method === "session/new");
  assert.deepEqual((newFwd!.params as Msg)._meta, {
    mine: true,
    claudeCode: { promptQueueing: true, emitRawSDKMessages: [{ type: "system", subtype: "init" }] },
  }, "the flag merges into _meta without dropping the client's own fields");

  await post(port, conn, { jsonrpc: "2.0", id: 3, method: "session/load", params: { sessionId: "S", cwd: "/proj" } });
  const loadFwd = agent().sent.map((s) => JSON.parse(s) as Msg).find((o) => o.method === "session/load");
  assert.deepEqual((loadFwd!.params as Msg)._meta, {
...
test("emitRawSDKMessages is NOT injected for a non-claude adapter", async () => {
  const { port, agent, close } = await makeTestServer();
  const a = sse(port);
  const conn = await a.conn;
  ...
  agent().emit(Buffer.from(JSON.stringify({
    jsonrpc: "2.0", id: initFwd!.id,
    result: { protocolVersion: 1, agentCapabilities: { loadSession: true } }, // no _meta.claudeCode
  })));
  await new Promise((r) => setTimeout(r, 20));

  await post(port, conn, { jsonrpc: "2.0", id: 2, method: "session/new", params: { cwd: "/proj", mcpServers: [] } });
  const newFwd = agent().sent.map((s) => JSON.parse(s) as Msg).find((o) => o.method === "session/new");
  assert.equal((newFwd!.params as Msg)._meta, undefined,
    "an adapter without agentCapabilities._meta.claudeCode gets no injection");

  a.close();
  await close();
});

test("emitRawSDKMessages _meta is also injected into the gateway's own revive session/load", async () => {
  // reviveThenForward synthesizes its own session/load (not the client's) when a
  // reaped session is touched again — the one injection point with no client
  // frame to merge into. Missing it here is exactly what silently breaks the
  // model correction after a session gets reaped and revived.
  ...
  await post(port, conn, { jsonrpc: "2.0", id: 2, method: "session/new", params: { cwd: "/proj", mcpServers: [] } });
  const newFwd = agent().sent.map((s) => JSON.parse(s) as Msg).find((o) => o.method === "session/new");
  agent().emit(Buffer.from(JSON.stringify({ jsonrpc: "2.0", id: newFwd!.id, result: { sessionId: "S" } })));
  await new Promise((r) => setTimeout(r, 20));

  // Idle it past the TTL so the sweep reaps it (session/close; the response
  // routes to CLOSE_SENTINEL and is dropped).
  reap(Date.now() + SESSION_IDLE_TTL_MS + 1_000);
  await new Promise((r) => setTimeout(r, 20));

  // Touching the reaped session with a prompt triggers the gateway's own revive
  // session/load, ahead of the prompt itself.
  await post(port, conn, {
    jsonrpc: "2.0", id: 3, method: "session/prompt", params: { sessionId: "S", prompt: [{ type: "text", text: "go" }] },
  });
  const reviveLoad = agent().sent.map((s) => JSON.parse(s) as Msg).filter((o) => o.method === "session/load").pop();
  assert.deepEqual((reviveLoad!.params as Msg)._meta, {
    claudeCode: { emitRawSDKMessages: [{ type: "system", subtype: "init" }] },
  }, "the gateway's own revive load must also ask for raw SDK messages");

  a.close();
  await close();
});

test("_claude/sdkMessage notifications never reach the ledger or a client", async () => {
  // Raw SDK message notifications are gateway-internal instrumentation (see the
  // _meta injection above) — one hook_response frame from this stream alone
  // measured 11KB, and one arrives on every turn, so they must never be appended
  // or broadcast.
  const { port, agent, close } = await makeTestServer();
  const a = sse(port);
  await a.conn;
  ...
  agent().emit(Buffer.from(JSON.stringify({
    ...
    params: { sessionId: "S", message: { type: "system", subtype: "hook_response", data: "x".repeat(100) } },
  })));
  const got = nextFrame(a, (o) => o.method === "session/update");
  agent().emit(Buffer.from(JSON.stringify({
    jsonrpc: "2.0", method: "session/update",
    params: { sessionId: "S", update: { sessionUpdate: "agent_message_chunk", content: { type: "text", text: "hi" } } },
  })));
  await withTimeout(got, 1000, "the real notification that followed should still arrive");

  const gotSdkMessage = a.frames.some((e) => {
    try { return parseFrame(e.data).method === "_claude/sdkMessage"; } catch { return false; }
  });
  assert.equal(gotSdkMessage, false, "a raw SDK message must never reach a client");

  // Never entering the ledger either: a fresh full replay must not see it, though
  // it does see the real session/update that followed it.
  const replay = sse(port, { lastEventId: "0" });
  await replay.conn;
  const replayGot = nextFrame(replay, (o) => o.method === "session/update");
  await withTimeout(replayGot, 1000, "the real notification should still replay");
  const replaySawIt = replay.frames.some((e) => {
    try { return parseFrame(e.data).method === "_claude/sdkMessage"; } catch { return false; }
  });
  assert.equal(replaySawIt, false, "a raw SDK message must never be appended to the ledger");
  replay.close();
...
// The real option list from an account seen in manual testing: family+version
// granularity varies per entry (an unversioned "opus[1m]" alias alongside a
// version-pinned "claude-fable-5[1m]"), which is exactly what a straight prefix
// match gets wrong — see resolveModelValue.
...
// Set up session "S" via session/new, with the agent replying a configOptions
// list containing exactly one `model` option (id:"model") at the given
// currentValue. Shared by the _claude/sdkMessage model-matching tests below.
async function newSessionWithModelOption(
  port: number, conn: string, agent: () => { sent: string[]; emit: (f: Buffer) => void },
  currentValue: string, options: Array<{ value: string; name: string }>,
): Promise<void> {
  await post(port, conn, { jsonrpc: "2.0", id: 1, method: "session/new", params: { cwd: "/proj", mcpServers: [] } });
  const newFwd = agent().sent.map((s) => JSON.parse(s) as Msg).find((o) => o.method === "session/new");
  agent().emit(Buffer.from(JSON.stringify({
    jsonrpc: "2.0", id: newFwd!.id,
    result: { sessionId: "S", configOptions: [{ id: "model", name: "Model", type: "select", currentValue, options }] },
  })));
  await new Promise((r) => setTimeout(r, 20));
}

test("resuming a session running Fable (unpinned) resolves via family+version, not just a prefix", async () => {
  // getAvailableModels() only resolves ANTHROPIC_MODEL / settings.json / models[0]
  // — a static guess — so EngineDock shows "Default (recommended)" for a resumed
  // session actually running something else. The raw SDK message's system-init
  // model is ground truth; the gateway maps it onto the cached model option's
  // real values and rebroadcasts config_option_update — but only when the
  // resolved value actually changed.
  const { port, agent, close } = await makeTestServer();
  const a = sse(port);
  const conn = await a.conn;
  await newSessionWithModelOption(port, conn, agent, "default", REAL_MODEL_OPTIONS);

  const gotUpdate = nextFrame(a, (o) => {
    if (o.method !== "session/update") return false;
    return ((o.params as Msg)?.update as Msg)?.sessionUpdate === "config_option_update";
  });
  agent().emit(Buffer.from(JSON.stringify({
    jsonrpc: "2.0", method: "_claude/sdkMessage",
    params: { sessionId: "S", message: { type: "system", subtype: "init", model: "claude-fable-5" } },
  })));
  const update = await withTimeout(gotUpdate, 1000, "the corrected config_option_update was never broadcast");
  const opts = ((update.params as Msg).update as Msg).configOptions as Array<Record<string, unknown>>;
  assert.equal(opts.find((o) => o.id === "model")?.currentValue, "claude-fable-5[1m]",
    "family opus/fable version 5 matches the version-5 option");

  // The same model on a later turn's init message must not rebroadcast — the
  // cache already matches, so nothing changed.
  const before = a.frames.filter((e) => { try { return parseFrame(e.data).method === "session/update"; } catch { return false; } }).length;
  agent().emit(Buffer.from(JSON.stringify({
    jsonrpc: "2.0", method: "_claude/sdkMessage",
    params: { sessionId: "S", message: { type: "system", subtype: "init", model: "claude-fable-5" } },
  })));
  await new Promise((r) => setTimeout(r, 30));
  const after = a.frames.filter((e) => { try { return parseFrame(e.data).method === "session/update"; } catch { return false; } }).length;
  assert.equal(after, before, "an unchanged model must not trigger a second broadcast");

  a.close();
  await close();
});

test("an unpinned session running Opus resolves to the version-agnostic alias, not the raw id", async () => {
  // "claude-opus-5[1m]" has no exact or prefix match in the option list at all
  // ("opus[1m]" isn't a prefix of it) — only family+version matching finds it,
  // via the option's empty version acting as a wildcard for "any Opus".
  const { port, agent, close } = await makeTestServer();
  const a = sse(port);
  const conn = await a.conn;
  await newSessionWithModelOption(port, conn, agent, "default", REAL_MODEL_OPTIONS);
  ...
  agent().emit(Buffer.from(JSON.stringify({
    jsonrpc: "2.0", method: "_claude/sdkMessage",
    params: { sessionId: "S", message: { type: "system", subtype: "init", model: "claude-opus-5[1m]" } },
  })));
  const update = await withTimeout(gotUpdate, 1000, "the corrected config_option_update was never broadcast");
  const opts = ((update.params as Msg).update as Msg).configOptions as Array<Record<string, unknown>>;
  assert.equal(opts.find((o) => o.id === "model")?.currentValue, "opus[1m]");

  a.close();
  await close();
});

test("a session pinned to Sonnet is left alone — the raw id must not overwrite an already-correct pin", async () => {
  // Regression guard: currentValue is already "sonnet" (the user picked it in
  // EngineDock, the option is checked). "claude-sonnet-5" resolves back to
  // "sonnet" (family match, the option carries no version), which equals the
  // cached currentValue, so nothing should broadcast — least of all the ugly
  // raw id "claude-sonnet-5" clobbering the user's pin and its checkmark.
  const { port, agent, close } = await makeTestServer();
  const a = sse(port);
  const conn = await a.conn;
  await newSessionWithModelOption(port, conn, agent, "sonnet", REAL_MODEL_OPTIONS);

  const before = a.frames.filter((e) => { try { return parseFrame(e.data).method === "session/update"; } catch { return false; } }).length;
  agent().emit(Buffer.from(JSON.stringify({
    jsonrpc: "2.0", method: "_claude/sdkMessage",
    params: { sessionId: "S", message: { type: "system", subtype: "init", model: "claude-sonnet-5" } },
  })));
  await new Promise((r) => setTimeout(r, 30));
  const after = a.frames.filter((e) => { try { return parseFrame(e.data).method === "session/update"; } catch { return false; } }).length;
  assert.equal(after, before, "a pin that already matches must not trigger a broadcast, let alone one with a raw id");

  a.close();
  await close();
});

test("a model id's version must not match an option pinned to a different version", async () => {
  // Same family, different version: "claude-opus-4-7" (version "4-7") must
  // resolve to the option that shares that exact version, never to a sibling
  // option pinned to version "5" — resolveModelValue's rule 2 excludes any
  // candidate whose version disagrees.
  const { port, agent, close } = await makeTestServer();
  const a = sse(port);
  const conn = await a.conn;
  const options = [
    { value: "opus-4-7", name: "Opus 4.7" },
    { value: "claude-opus-5[1m]", name: "Opus 5 (1M context)" },
  ];
  await newSessionWithModelOption(port, conn, agent, "default", options);
  ...
  agent().emit(Buffer.from(JSON.stringify({
    jsonrpc: "2.0", method: "_claude/sdkMessage",
    params: { sessionId: "S", message: { type: "system", subtype: "init", model: "claude-opus-4-7" } },
  })));
  const update = await withTimeout(gotUpdate, 1000, "the corrected config_option_update was never broadcast");
  const opts = ((update.params as Msg).update as Msg).configOptions as Array<Record<string, unknown>>;
  assert.equal(opts.find((o) => o.id === "model")?.currentValue, "opus-4-7",
    "version 4-7 must match the version-4-7 option, not the version-5 sibling");
