...
// Full-screen general shell (like ttyd). Unlike LoginTerminal, closing this
// view must NOT stop the session — the PTY on the gateway host is meant to
// survive a dropped connection so reopening picks the same shell back up
// (the gateway reaps it on its own after an idle TTL with no subscriber).
export function Terminal({ cwd, onClose }: { cwd?: string; onClose: () => void }) {
  ...
  return (
    <div className="term-overlay" role="dialog" aria-label="Terminal">
